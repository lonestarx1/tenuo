...
class A2AServer:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            require_warrant: Reject tasks without warrant
            require_audience: Require aud claim matches our URL
            check_replay: Enforce jti uniqueness
            replay_window: Seconds to remember jti values
            max_chain_depth: Maximum delegation chain depth
            audit_log: Destination for audit events (file, callable, or stderr)
        ...
        self.require_warrant = require_warrant
        self.require_audience = require_audience
        self.check_replay = check_replay
        self.replay_window = replay_window
        self.max_chain_depth = max_chain_depth
        self.audit_log = audit_log if audit_log is not None else sys.stderr
    ...
    async def _validate_chain(
        ...
        if root_issuer not in self.trusted_issuers:
            raise UntrustedIssuerError(
                root_issuer,
                reason=f"Root warrant issuer '{root_issuer}' is not trusted"
            )
        ...
        for i in range(len(chain_warrants)):
            ...
            if i > 0:
                ...
                parent_holder = (
                    getattr(parent, "sub", None) or
                    getattr(parent, "subject", None) or
                    getattr(parent, "authorized_holder", None)
                )
                
                # Get current warrant's issuer
                current_issuer = (
                    getattr(current, "iss", None) or
                    getattr(current, "issuer", None)
                )
                ...
                if not self._grants_are_subset(current, parent):
                    raise ChainValidationError(
                        f"Monotonicity violation at position {i}: "
                        f"child grants exceed parent grants",
                        depth=i,
                    )
        ...
        logger.debug(
            f"Chain validated: {len(chain_warrants)} warrants, "
            f"root issuer: {root_issuer}"
        )
    ...
    def get_agent_card_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            } if card.tenuo_extension else None,
        }
    ...
    def _create_app(self):
        ...
        try:
            ...
            raise ImportError(
                "starlette is required for A2A server. "
